...
HIAI_StatusT HIAI_InitAndStartGraph()
{
    ...
    graph->SetDataRecvFunctor(target_port_config,
        std::shared_ptr<DetectionSSDDataRecvInterface>(
            new DetectionSSDDataRecvInterface(TEST_DEST_DATA_BBOX_PATH)));

    target_port_config.port_id = DEST_PORT_ID_1;
    graph->SetDataRecvFunctor(target_port_config,
        std::shared_ptr<DetectionSSDDataRecvInterface>(
            new DetectionSSDDataRecvInterface(TEST_DEST_DATA_NUM_PATH)));
    return HIAI_OK;
}
